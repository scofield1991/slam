...
Mat image0, image1;
UMat oclImage0, oclImage1;
Mat imageShow, harrisLast;
UMat oclImageShow, oclHarrisLast;
...
static void download(const UMat& ocl_mat, vector<Point2f>& vec)
{
...
static void download(const UMat& ocl_mat, vector<unsigned char>& vec)
{
  vec.resize(ocl_mat.cols);
...
void imageDataHandler(const sensor_msgs::Image::ConstPtr& imageData) 
{
  ...
  if (totalFeatureNum > 0) {
    ...
    UMat oclFeaturesLast(featuresLastMat);
    UMat oclFeaturesCur, oclFeaturesStatus;
